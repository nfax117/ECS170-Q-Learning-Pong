...
if os.stat("model_pretrained.pth").st_size != 0:
...
#scale epsilon_start after training it (hardcode it)
epsilon_start = 1.0
epsilon_final = 0.01
epsilon_decay = 30000
...
for frame_idx in range(1, num_frames + 1):
    ...
    # Every 50000 frames, Write to output file losses and all rewards, and save state.
    if frame_idx % 50000 == 0:
        ...
        # Save all_rewards and losses
        file1 = open("all_rewards.pkl", 'wb')
        pickle.dump(all_rewards, file1)
        file1.close()
        print("All rewards has been saved!!!")
        
        file2 = open("losses.pkl", 'wb')
        pickle.dump(losses, file2)
        file2.close()
        print("Losses has been saved!!!")

        # with open('all_rewards.pkl', 'rb') as handle:
